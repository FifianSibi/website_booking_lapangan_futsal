<aside class="main-sidebar sidebar-dark-primary elevation-4">
    <!-- Brand Logo -->
    <a href="index3.html" class="brand-link">
        <img src="<?= base_url() ?>/img/AdminLTELogo.png" alt="AdminLTE Logo" class="brand-image img-circle elevation-3" style="opacity: .8">
        <span class="brand-text font-weight-light">AdminLTE 3</span>
    </a>

    <!-- Sidebar -->
    <div class="sidebar">
        <!-- Sidebar user panel (optional) -->
        <!-- View -->
        <div class="user-panel mt-3 pb-3 mb-3 d-flex">
            <?php if (session()->has('user_id')) : ?>
                <?php
                $profileModel = new \App\Models\ProfileModel();
                $userId = session()->get('user_id');
                $profile = $profileModel->getProfileByUserId($userId);
                ?>
                <?php if ($profile) : ?>
                    <div class="image">
                        <?php
                        // Path foto default
                        $photoPath = base_url() . '/img/user2-160x160.jpg';

                        // Jika ada foto dalam profil, gunakan foto tersebut
                        if (isset($profile['photo']) && $profile['photo']) {
                            $photoPath = base_url('uploads/profiles/' . $userId . '/' . $profile['photo']);
                        }
                        ?>
                        <img src="<?= $photoPath ?>" class="img-circle elevation-2" alt="User Image">
                    </div>
                    <div class="info">
                        <a href="#" class="d-block"><?= $profile['first_name'] . ' ' . $profile['last_name']; ?></a>
                    </div>
                <?php else : ?>
                    <div class="image">
                        <img src="<?= base_url() ?>/img/user2-160x160.jpg" class="img-circle elevation-2" alt="User Image">
                    </div>
                <?php endif; ?>
            <?php endif; ?>
        </div>




        <!-- SidebarSearch Form -->
        <div class="form-inline">
            <div class="input-group" data-widget="sidebar-search">
                <input class="form-control form-control-sidebar" type="search" placeholder="Search" aria-label="Search">
                <div class="input-group-append">
                    <button class="btn btn-sidebar">
                        <i class="fas fa-search fa-fw"></i>
                    </button>
                </div>
            </div>
        </div>

        <!-- Sidebar Menu -->
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                <!-- Add icons to the links using the .nav-icon class
                with font-awesome or any other icon font library -->
                <li class="nav-item">
                    <a href="/admin/dashboard" class="nav-link">
                        <i class="nav-icon fas fa-tachometer-alt"></i>
                        <p>
                            Dashboard
                        </p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="/adminlapangan" class="nav-link">
                        <i class="nav-icon fas fa-futbol" aria-hidden="true"></i>
                        <p>
                            lapangan
                        </p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="/adminprofile" class="nav-link">
                        <i class="nav-icon fas fa-users"></i>
                        <p>
                            profile
                        </p>
                    </a>
                </li>
                <li class="nav-item">
                    <a href="/adminreservasi" class="nav-link">
                        <i class="nav-icon fas fa-file"></i>
                        <p>
                            Reservasi
                        </p>
                    </a>
                </li>
            </ul>
        </nav>
        <!-- /.sidebar-menu -->
    </div>
    <!-- /.sidebar -->
</aside>